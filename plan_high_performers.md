...
- [x] ROE 임계값 이상인 종목만 반환한다
- [x] ROE 기준 내림차순 정렬된 결과를 반환한다
- [x] 임계값 이상인 종목이 없으면 빈 DataFrame을 반환한다
- [x] 분석 결과가 없으면 빈 DataFrame을 반환한다
- [x] 통계(평균/최대/최소)를 올바르게 계산한다
- [x] ROE 구간별 분포를 올바르게 계산한다
